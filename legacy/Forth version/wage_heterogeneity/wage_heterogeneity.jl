...
cd(@__DIR__)
using CSV, DataFrames, Statistics, Plots, Statistics, Dates, Downloads, SHA
...
function check_data_integrity(path, hash)
    if !isfile(path)
        error("File not found: $path. Please ensure the dataset is in the working directory.")
    end

    actual_hash = bytes2hex(open(sha256, path))

    if actual_hash == hash
        println("Success: File exists and SHA256 hash matches.")
        return true
    else
        error("Integrity error: Hash mismatch! Expected $hash but got $actual_hash.")
    end
end

# Run check
check_data_integrity(local_path, expected_hash)
