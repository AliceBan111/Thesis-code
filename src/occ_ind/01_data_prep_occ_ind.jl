# =============================================================================
# 01_data_prep_occ_ind.jl
# Oil Supply News Shock × Occupation-within-Industry Heterogeneity
#
# Builds an (occ_group × ind_group × date) panel.
# Industry classification uses the original 12-group ind1990 mapping;
# the 4-way merge into Energy / Manufacturing&Construction / Trade / Services
# is handled downstream in 02_lp_estimation_occ_ind.jl.
#
# main(variant) returns a single DataFrame with columns:
#   occ_group, ind_group, date, year, month,
#   unemp_rate, log_emp_count, pop_weight,
#   [variant-specific outcome columns],
#   shock, shock_lag1..L_LAG, log_oil_lag1, ffr_lag1,
...
function get_output_dir(variant::Symbol)::String
    base = joinpath(@__DIR__, "../..", "result", "occ_ind")
    dirs = Dict(
        :hourly_rate     => "hourly_rate",
        :hours           => "hours",
        :income          => "income",
        :income_share_var=> "income_share",
        :inequality      => "inequality",
        :median          => "median",
        :unemployment    => "unemployment",
        :employment      => "employment",
    )
    haskey(dirs, variant) || error("Unknown variant: $variant")
    dir = joinpath(base, dirs[variant])
    mkpath(dir)
    return dir
end
...
# =============================================================================
# PARSE CPS FIXED-WIDTH FILE
# Column layout (cps_00018.dat):
#   YEAR 1-4 | MONTH 5-6 | WTFINL 7-20 | EARNWEEK2 21-28 | AGE 29-30
#   SEX 31 | MARST 32 | EMPSTAT 33-34 | OCC1990 35-37 | IND1990 38-40
#   CLASSWKR 41-42 | UHRSWORKT 43-45 | AHRSWORKT 46-48 | EARNWT 49-58
# =============================================================================
function parse_cps(path::String)::DataFrame
    println("Parsing CPS: $path  (", round(filesize(path)/1e9, digits=2), " GB)")
...
function weighted_quantile(x::AbstractVector, w::AbstractVector, q::Float64)::Float64
    idx = sortperm(x); xs = x[idx]; ws = w[idx]
    cumw = cumsum(ws); cumw ./= cumw[end]
    return xs[clamp(searchsortedfirst(cumw, q), 1, length(xs))]
end

function build_panel_unemp(cps::DataFrame)::DataFrame
    gdf = groupby(cps, [:occ_group, :ind_group, :date, :year, :month])
    panel = combine(gdf) do sdf
        emp = sdf.empstat; wt = sdf.wtfinl
    ...
    return sort(panel, [:occ_group, :ind_group, :date])
end
...
function build_panel_earn(cps::DataFrame, variant::Symbol)::DataFrame
    ...
    elseif variant in (:income, :unemployment, :employment)
        combine(gdf,
            [:log_rincome, :earnwt] => ((w,wt) -> sum(w.*wt)/sum(wt)) => :log_rincome,
            [:log_rwage,   :earnwt] => ((x,wt) -> sum(x.*wt)/sum(wt)) => :log_rwage,
            [:age,         :earnwt] => ((x,wt) -> sum(x.*wt)/sum(wt)) => :age_mean,
...
Builds and returns the (occ_group × ind_group × date) panel for the given
variant. The 4-way industry merge is applied downstream in the LP script.
"""
function main(variant::Symbol = :hourly_rate)
    println("="^60)
    println("occ_ind data prep | variant: $variant")
    println("="^60)
    ...
    panel_all   = leftjoin(panel_labor, panel_earn,
                           on=[:occ_group, :ind_group, :date, :year, :month])
    ...
    panel = dropmissing(panel, [:shock, :log_oil_lag1, :ffr_lag1,
                                :cpi_lag1, :indpro_lag1, :t10y3m_lag1])
    sort!(panel, [:occ_group, :ind_group, :date])

    println("\nDone: $(nrow(panel)) obs | ",
            "$(length(unique(panel.occ_group))) occ groups | ",
            "$(length(unique(panel.ind_group))) ind groups")
    return panel
end