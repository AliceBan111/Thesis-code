...
# =============================================================================
# 6. RUN THREE-TASK REGRESSION
# =============================================================================

function run_three_task_regression(df::DataFrame)
    df_reg = copy(df)
    df_reg.industry = categorical(df_reg.industry)

    model = lm(
        @formula(cumulative_beta ~ task_abstract + task_routine + task_manual + industry),
        df_reg
    )
...
function save_regression_table(model, outcome::String, output_dir::String)
    ct = coeftable(model)

    out = DataFrame(
        outcome = outcome,
        model = "three_tasks_industry_FE",
    ...
    path = joinpath(output_dir, "$(outcome)_three_task_industry_FE_results.csv")
    CSV.write(path, out)
...
function process_one_outcome(
    ...
    # Remove NaN / Inf that dropmissing! does not remove.
    filter!(
    ...
    model = run_three_task_regression(df)
    reg_table = save_regression_table(model, outcome, output_dir)
...
function build_summary_table(output_dir::String = OUTPUT_DIR)
    ...
    for outcome in OUTCOME_LIST
        ...
        for term in ["task_abstract", "task_routine", "task_manual"]
            sub = filter(row -> row.term == term, df)
    ...
    output_path = joinpath(output_dir, "summary_three_tasks_with_industry_FE.csv")
...
function main()
    ...
    println("\n============================================================")
    println("Summary: cumulative_beta ~ abstract + routine + manual + industry FE")
    println("============================================================")
