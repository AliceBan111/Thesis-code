...
function get_global_ylims(irfs::Dict{Int, DataFrame})
    ...
    for g in keys(IND_LABELS)
        ...
        df = irfs[g]
        # 排除可能存在的 NaN
        valid_min = minimum(filter(!isnan, df[!, :ci_lo90]))
        valid_max = maximum(filter(!isnan, df[!, :ci_hi90]))
...
function plot_absolute_irf(irfs::Dict{Int, DataFrame}, pw_bh::DataFrame, variant::Symbol, output_dir::String)
    ...
    for (idx, g) in enumerate(group_ids)
        ...
        # 使用专属颜色绘制主线
        lines!(ax, df.horizon, df.beta, color = base_color, linewidth = 2.5)
        ...
        if nrow(sig_sub) > 0
            ...
            scatter!(ax, sig_points.horizon, sig_points.beta, 
