...
function extract_irf(df::DataFrame)
    ...
    for g in keys(IND_LABELS)
        ...
        out[g] = sub[:, [:horizon,:beta,:se,:ci_lo90,:ci_hi90,:ci_lo68,:ci_hi68]]
    end
    return out
end
...
#
#   Step 1: Compute the cumulative IRF for each ind_group (h=1:36)
#   Step 2: Run the cross-sectional regression cumIRF_g = α + δ·OilIntensity_g + u_g
#   Step 3: Report R², δ, and the variance share
#
#   Also accepts occ_group-level oil_exposure (Bartik); the structure is the same
# =============================================================================
...
Run the cross-sectional OilIntensity regression at the industry level, and report R² and the variance share.
ind_intensity_df is the third return value from main() in data_prep_ind_4.jl.
"""
...
occ_irfs: IRFs from the occupation-version run_lp, keyed by occ_group.
oilshare_df: the second return value from main() in data_prep_ind_4.jl.
"""
