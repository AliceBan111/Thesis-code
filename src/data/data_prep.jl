...
const OUTPUT_DIR = joinpath(@__DIR__, "../..", "result")
mkpath(OUTPUT_DIR)

const CPS_FILE_ID  = "1rYTH9WjOYUFA9sYsUjLmFvRe8wMH-PJU"
const CPS_SHA256   = "a7065bba4cc7fb0ca541cc5587b5c3fb74477f63cb1d5d0f61f5f7011adb754d"
const CPS_PATH     = joinpath(DATA_DIR, "cps_00012.dat")
...
#   IND1990    16-18
#   UHRSWORKT  19-21
#   EARNWT     22-31  (4 implied decimals)
#   EARNWEEK   33-40  (2 implied decimals)
...
function parse_cps(path::String)::DataFrame
    ...
    open(path, "r") do f
        for line in eachline(f)
            ...
            # ── EARNWT：tryparse + missing 跳过 ──
            earnwt_raw = tryparse(Float64, strip(@view line[22:31]))
            isnothing(earnwt_raw) && continue

            # ── EARNWEEK：修复原始 bug（parse 遇空串崩溃 → tryparse + 空串检查）──
            earnwk_str = strip(@view line[33:40])
            ...
            if n > length(year_v)
                ...
                foreach(v -> resize!(v, new_cap),
                    (year_v, month_v, age_v, sex_v, marst_v,
                     empstat_v, occ1990_v, ind1990_v, earnwt_v, earnweek_v))
            end

            # ── 写入（剩余字段只在通过过滤后才解析）──
            year_v[n]     = year
            month_v[n]    = month
            age_v[n]      = parse(Int32, @view line[7:8])
            sex_v[n]      = parse(Int32, @view line[9:9])
            marst_v[n]    = parse(Int32, @view line[10:10])
            empstat_v[n]  = parse(Int32, @view line[11:12])
            occ1990_v[n]  = parse(Int32, @view line[13:15])
...
# =============================================================================
function clean_cps(df::DataFrame)::DataFrame
    println("Applying sample restrictions...")
    ...
    # 6f. Demographic dummies
    df[!, :female]  = Int.(df.sex .== 2)
    df[!, :married] = Int.(df.marst .== 1)   # MARST=1: married, spouse present

    # 6g. Date variable（用整数列拼，不依赖字符串格式）
    df[!, :date] = Date.(df.year, df.month, 1)

    println("  After restrictions: ", nrow(df), " observations")
    return df
end
...
function deflate_wages!(cps::DataFrame, cpi::DataFrame)::DataFrame
    ...
    cps = @subset(cps, .!ismissing.(:cpi_norm))
    cps[!, :real_earnweek] = cps.earnweek ./ cps.cpi_norm
    cps[!, :log_rwage]     = log.(cps.real_earnweek)
    return cps
end
...
# =============================================================================
function build_cell_panel(cps::DataFrame)::DataFrame
    ...
    println("  Total cell-month observations: ", nrow(panel))
    println("  Unique cells: ", length(unique(panel.cell_id)))
...
function build_macro_panel(shock_df, oil_df, ffr_df, unrate_df)::DataFrame
...
function main()
    ...
    cpi_df    = load_cpi()
    cps_clean = clean_cps(cps_raw)
    cps_clean = deflate_wages!(cps_clean, cpi_df)

    # Build cell panel
    cell_panel = build_cell_panel(cps_clean)
    ...
    # Merge cell panel with macro
    panel = leftjoin(cell_panel, macro_df, on = :date)
    panel = dropmissing(panel, [:shock, :log_oil_lag1, :ffr_lag1, :unrate_lag1])
