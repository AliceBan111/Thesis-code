# =============================================================================
# 05_plots.jl
# IRF plots for all industry groups
# =============================================================================
...
include("03_significance_tests_ind.jl")
...
function plot_absolute_irf(irfs::Dict{Int, DataFrame}, pw_bh::DataFrame;
    ...
    for (idx, g) in enumerate(1:13)
        ...
        ax  = Axis(fig[row, col],
                ...
                #    ylabel = "Log wage change",
                   ylabel = "Log income change",
                   titlesize = 11)
        # ylims!(ax, -0.04, 0.04) # income
        # CairoMakie.ylims!(ax, -0.01, 0.01) # hours, unemployment
        # CairoMakie.ylims!(ax, -0.03, 0.03) # hourly_rate, inequality, median
        # CairoMakie.ylims!(ax, -0.005, 0.005) #income share
        CairoMakie.ylims!(ax, -0.02, 0.02)
        ...
        sig_pts = @subset(pw_bh, :ind_group .== g, :bh_reject .== true)
        if nrow(sig_pts) > 0
            sig_vals = innerjoin(
                        select(sig_pts, :horizon, :ind_group),
    ...
    # Shared legend note
    Label(fig[5, 1:4],
          "Shaded bands: 68%, 90% and 95% block-bootstrap confidence intervals (B=$N_BOOT, block=$BLOCK_SIZE months). ★ = BH-significant (q=0.05).",
...
#                    titlesize = 10)
#         ylims!(ax, -0.04, 0.04)  # income, median
...
function plot_persistence_summary(sig_table::DataFrame;
    ...
    ax  = Axis(fig[1, 1],
               ...
               xlabel = "p-value",
               title  = "Persistence Classification by Industry Group")

    for (i, row) in enumerate(eachrow(sig_table))
        c = get(type_colors, row.persistence_type, :gray)
        CairoMakie.scatter!(ax, [row.p_short], [i]; color = c, marker = :circle, markersize = 12,
                 label = "Short-run")
        CairoMakie.scatter!(ax, [row.p_long],  [i]; color = c, marker = :diamond, markersize = 12,
                 label = "Long-run")
