...
include("data_prep_ind_unemployment.jl")   # H_MAX, L_LAG, IND_LABELS, OUTPUT_DIR
...
function build_lp_data(panel::DataFrame, h::Int)::Union{DataFrame, Nothing}
    ...
    for g in gdf
        ...
        lead_unemp = Vector{Union{Float64,Missing}}(missing, n)
        h < n && (lead_unemp[1:n-h] = sub.unemp_rate[h+1:n])

        lag_unemp = Vector{Union{Float64,Missing}}(missing, n)
        lag_unemp[2:n] = sub.unemp_rate[1:n-1]

        sub[!, :dep_var]  = lead_unemp .- lag_unemp
        sub[!, :lag_unemp] = lag_unemp
        push!(chunks, sub)
    end
...
function build_col_lists!(df_h::DataFrame)
    ...
    cell_cols  = [:log_rincome, :log_rwage, :age_mean, :female_share, :married_share, :hours_mean]
