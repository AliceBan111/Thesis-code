...
include("data_prep_ind_hourlyrate.jl")   # H_MAX, L_LAG, IND_LABELS, OUTPUT_DIR
...
function build_lp_data(panel::DataFrame, h::Int)::Union{DataFrame, Nothing}
    ...
    for g in gdf
        ...
        lead_hourlyrate = Vector{Union{Float64,Missing}}(missing, n)
        h < n && (lead_hourlyrate[1:n-h] = sub.log_rwage[h+1:n])

        lag_hourlyrate = Vector{Union{Float64,Missing}}(missing, n)
        lag_hourlyrate[2:n] = sub.log_rwage[1:n-1]

        sub[!, :dep_var]  = lead_hourlyrate .- lag_hourlyrate
        sub[!, :lag_hourlyrate] = lag_hourlyrate
        push!(chunks, sub)
    end
