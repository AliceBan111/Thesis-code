...
include("data_prep_ind_median.jl")   # H_MAX, L_LAG, IND_LABELS, OUTPUT_DIR
...
function build_lp_data(panel::DataFrame, h::Int)::Union{DataFrame, Nothing}
    ...
    for g in gdf
        ...
        lead_med = Vector{Union{Float64,Missing}}(missing, n)
        h < n && (lead_med[1:n-h] = sub.log_rincome_p50[h+1:n])

        lag_med = Vector{Union{Float64,Missing}}(missing, n)
        lag_med[2:n] = sub.log_rincome_p50[1:n-1]

        sub[!, :dep_var]  = lead_med .- lag_med
        sub[!, :lag_med] = lag_med
        push!(chunks, sub)
    end
...
function build_col_lists!(df_h::DataFrame)
    ...
    cell_cols  = [:log_rwage_p50, :age_mean, :female_share, :married_share, :hours_mean, :unemp_rate]
