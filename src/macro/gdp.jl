...
const OUTPUT_DIR = joinpath(@__DIR__, "../../result/macro/gdp")
mkpath(OUTPUT_DIR)
...
function load_oil_shock_macro()::DataFrame
    ...
    df_monthly = DataFrame(date = dates, shock = shocks)
    df_monthly = @subset(df_monthly, DATE_START .<= :date .<= DATE_END)
    sort!(df_monthly, :date)

    df_monthly[!, :year] = year.(df_monthly.date)
    df_monthly[!, :month] = month.(df_monthly.date)

    df_monthly[!, :quarter] = ceil.(Int, df_monthly.month / 3)

    df_quarterly = combine(groupby(df_monthly, [:year, :quarter]), :shock => sum => :shock)

    df_quarterly[!, :date] = Date.(
        df_quarterly.year, 
        (df_quarterly.quarter .- 1) .* 3 .+ 1,  # Q1->1, Q2->4, Q3->7, Q4->10
        1
    )

    df = df_quarterly[:, [:date, :shock]]
    sort!(df, :date)
...
function load_gdp_log()::DataFrame
    println("Loading GDP and taking log...")
    path = joinpath(DATA_DIR, "GDPC1.csv")
    df = CSV.read(path, DataFrame;
        types=Dict(1 => Date, 2 => Float32), dateformat="yyyy-m-d")
    rename!(df, first(names(df)) => :date, names(df)[2] => :gdp)
    df = filter(row -> DATE_START <= row.date <= DATE_END, df)
    sort!(df, :date)
...
# =============================================================================
# 3.5 HELPER: month -> quarter
# =============================================================================
function aggregate_monthly_to_quarterly(df::DataFrame, col::Symbol)::DataFrame
    tmp = copy(df)
    # 计算季度起始日期 (Q1->01-01, Q2->04-01, Q3->07-01, Q4->10-01)
    tmp[!, :q_date] = Date.(year.(tmp.date), (ceil.(Int, month.(tmp.date) / 3) .- 1) .* 3 .+ 1, 1)
    # 按季度分组并取均值
    res = combine(groupby(tmp, :q_date), col => mean => col)
    rename!(res, :q_date => :date)
    return res
end
...
# 4. BUILD MACRO PANEL
# =============================================================================
function build_gdp_panel()::DataFrame
    println("Building GDP macro panel...")
    ...
    # 关键：月度变量先转为季度频（取季度均值）
    oil_q = aggregate_monthly_to_quarterly(oil_df, :log_oil_price)
    ffr_q = aggregate_monthly_to_quarterly(ffr_df, :fedfunds)

    # 合并所有季度数据
    panel = outerjoin(gdp_df, shock_df, on = :date)
    panel = outerjoin(panel, oil_q,    on = :date)
    panel = outerjoin(panel, ffr_q,    on = :date)
    sort!(panel, :date)
    ...
    panel[!, :log_gdp_lag1] = lag_vec(panel.log_gdp, 1)

    # 剔除含缺失值的行（仅保留首尾自然缺失外的有效样本）
    required_cols = vcat([:log_gdp, :shock, :log_oil_lag1, :ffr_lag1],
                         [Symbol("shock_lag", l) for l in 1:L_LAG])
    panel = dropmissing(panel, required_cols)
    sort!(panel, :date)
    
    println("  Panel rows: ", nrow(panel))  # 修复后应显示 150~170 行左右
    return panel
end

# =============================================================================
# 5. BUILD LP DATA FOR HORIZON h
#    dep_var_t = log_gdp_{t+h} - log_gdp_{t-1}
# =============================================================================
function build_lp_data_gdp(panel::DataFrame, h::Int)::Union{DataFrame, Nothing}
    df = sort(copy(panel), :date)
    n  = nrow(df)

    lead_gdp = Vector{Union{Float64, Missing}}(missing, n)
    h < n && (lead_gdp[1:n-h] = df.log_gdp[h+1:n])

    lag_gdp = Vector{Union{Float64, Missing}}(missing, n)
    lag_gdp[2:n] = df.log_gdp[1:n-1]

    df[!, :dep_var]  = lead_gdp .- lag_gdp  
    df[!, :lag_gdp]  = lag_gdp

    required = vcat([:dep_var, :shock, :log_oil_lag1, :ffr_lag1, :lag_gdp],
                    [Symbol("shock_lag", l) for l in 1:L_LAG])
...
function build_x_cols()::Vector{Symbol}
    ...
    macro_cols = [:log_oil_lag1, :ffr_lag1, :lag_gdp]
...
# =============================================================================
function block_bootstrap_gdp(panel::DataFrame, h::Int;
                               n_boot::Int     = N_BOOT,
                               block_size::Int = BLOCK_SIZE,
                               rng::AbstractRNG = Random.default_rng())

    df_h = build_lp_data_gdp(panel, h)
...
# 9. RUN FULL LP
# =============================================================================
function run_full_lp_gdp(panel::DataFrame)::Tuple{DataFrame, DataFrame}
    println("\nRunning GDP LP  h = 0 … $H_MAX")
    ...
    for h in 0:H_MAX
        print("\r  h = $h / $H_MAX  ")
        out = block_bootstrap_gdp(panel, h; n_boot = N_BOOT,
...
# =============================================================================
function main()
    panel = build_gdp_panel()

    results_df, irf_df = run_full_lp_gdp(panel)

    CSV.write(joinpath(OUTPUT_DIR, "lp_gdp_coefficients.csv"), results_df)
    CSV.write(joinpath(OUTPUT_DIR, "lp_gdp_irf.csv"),          irf_df)
