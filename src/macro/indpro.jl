...
const OUTPUT_DIR = joinpath(@__DIR__, "../../result/macro/indpro")
mkpath(OUTPUT_DIR)
...
function load_indpro_log()::DataFrame
    println("Loading INDPRO and taking log...")
    path = joinpath(DATA_DIR, "INDPRO.csv")
    df = CSV.read(path, DataFrame;
        types=Dict(1 => Date, 2 => Float32), dateformat="yyyy-m-d")
    rename!(df, first(names(df)) => :date, names(df)[2] => :indpro)
    df = filter(row -> DATE_START <= row.date <= DATE_END, df)
    sort!(df, :date)
    
    # 取对数
    df[!, :log_indpro] = log.(df.indpro)
...
#    dep_var_t = log_indpro_{t+h} - log_indpro_{t-1}
# =============================================================================
function build_lp_data_indpro(panel::DataFrame, h::Int)::Union{DataFrame, Nothing}
    df = sort(copy(panel), :date)
    n  = nrow(df)

    lead_indpro = Vector{Union{Float64, Missing}}(missing, n)
    h < n && (lead_indpro[1:n-h] = df.log_indpro[h+1:n])

    lag_indpro = Vector{Union{Float64, Missing}}(missing, n)
    lag_indpro[2:n] = df.log_indpro[1:n-1]
...
function build_x_cols()::Vector{Symbol}
    ...
    macro_cols = [:log_oil_lag1, :ffr_lag1, :lag_indpro]
...
# =============================================================================
function block_bootstrap_indpro(panel::DataFrame, h::Int;
                               n_boot::Int     = N_BOOT,
                               block_size::Int = BLOCK_SIZE,
                               rng::AbstractRNG = Random.default_rng())

    df_h = build_lp_data_indpro(panel, h)
...
# 9. RUN FULL LP
# =============================================================================
function run_full_lp_indpro(panel::DataFrame)::Tuple{DataFrame, DataFrame}
    println("\nRunning indpro LP  h = 0 … $H_MAX")
    ...
    for h in 0:H_MAX
        print("\r  h = $h / $H_MAX  ")
        out = block_bootstrap_indpro(panel, h; n_boot = N_BOOT,
...
# =============================================================================
function main()
    panel = build_indpro_panel()

    results_df, irf_df = run_full_lp_indpro(panel)

    CSV.write(joinpath(OUTPUT_DIR, "lp_indpro_coefficients.csv"), results_df)
    CSV.write(joinpath(OUTPUT_DIR, "lp_indpro_irf.csv"),          irf_df)
