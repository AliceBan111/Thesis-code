"""
plot_cpi_irf.py
Reads lp_cpi_irf.csv and plots the impulse response of log CPI
to a 1-unit oil supply news shock, with 68 / 90 / 95 % CIs.
"""
import pandas as pd
...
IRF_PATH    = os.path.join(SCRIPT_DIR, "../../result/macro/cpi/lp_cpi_irf.csv")
OUTPUT_PATH = os.path.join(SCRIPT_DIR, "../../result/macro/cpi/lp_cpi_irf.pdf")
...
def plot_irf(irf_path: str, output_path: str) -> None:
    ...
    ax.set_ylabel("Response of log CPI\n(cumulative from t−1)", fontsize=11)
    ax.set_title("IRF: Oil Supply News Shock → CPI", fontsize=13, fontweight="medium")
    ax.set_xlim(h[0], h[-1])
