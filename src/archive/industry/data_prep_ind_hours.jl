...
const OUTPUT_DIR = joinpath(@__DIR__, "../..", "result", "ind", "hours")
mkpath(OUTPUT_DIR)
            ...
            if n > length(year_v)
                ...
                foreach(v -> resize!(v, new_cap),
                    (year_v, month_v, wtfinl_v, age_v, sex_v, marst_v,
                     empstat_v, occ1990_v, classwkr_v, uhrsworkt_v, earnwt_v, earnweek_v, ind1990_v, classwkr_v))
            end
...
function build_panel_earn(cps::DataFrame)::DataFrame
    ...
    panel = combine(gdf,
        ...
        [:log_uhrsworkt, :earnwt] =>
            ((x, wt) -> sum(x .* wt) / sum(wt)) => :log_hours,
        [:age, :earnwt] =>
