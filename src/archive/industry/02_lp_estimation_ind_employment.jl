...
function build_lp_data(panel::DataFrame, h::Int)::Union{DataFrame, Nothing}
    ...
    for g in gdf
        ...
        lead_emp = Vector{Union{Float64,Missing}}(missing, n)
        h < n && (lead_unemp[1:n-h] = sub.log_emp_count[h+1:n])

        lag_emp = Vector{Union{Float64,Missing}}(missing, n)
        lag_unemp[2:n] = sub.log_emp_count[1:n-1]

        sub[!, :dep_var]  = lead_emp .- lag_emp
        sub[!, :lag_emp] = lag_emp
        push!(chunks, sub)
    end
