...
include("data_prep_ind_hours.jl")   # H_MAX, L_LAG, IND_LABELS, OUTPUT_DIR
...
function build_lp_data(panel::DataFrame, h::Int)::Union{DataFrame, Nothing}
    ...
    for g in gdf
        ...
        lead_hours = Vector{Union{Float64,Missing}}(missing, n)
        h < n && (lead_hours[1:n-h] = sub.log_hours[h+1:n])

        lag_hours = Vector{Union{Float64,Missing}}(missing, n)
        lag_hours[2:n] = sub.log_hours[1:n-1]

        sub[!, :dep_var]  = lead_hours .- lag_hours
        sub[!, :lag_hours] = lag_hours
        push!(chunks, sub)
    end
...
function build_col_lists!(df_h::DataFrame)
    ...
    cell_cols  = [:age_mean, :female_share, :married_share, :unemp_rate]
