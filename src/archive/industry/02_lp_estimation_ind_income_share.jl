...
include("data_prep_ind_income.jl")   # H_MAX, L_LAG, IND_LABELS, OUTPUT_DIR
...
function build_lp_data(panel::DataFrame, h::Int)::Union{DataFrame, Nothing}
    ...
    for g in gdf
        ...
        lead_income_share = Vector{Union{Float64,Missing}}(missing, n)
        h < n && (lead_income_share[1:n-h] = sub.income_share[h+1:n])

        lag_income_share = Vector{Union{Float64,Missing}}(missing, n)
        lag_income_share[2:n] = sub.income_share[1:n-1]

        sub[!, :dep_var]  = lead_income_share .- lag_income_share
        sub[!, :lag_income_share] = lag_income_share
        push!(chunks, sub)
    end
...
function build_col_lists!(df_h::DataFrame)
    ...
    cell_cols  = [:age_mean, :female_share, :married_share, :hours_mean, :unemp_rate]
...
function extract_irf(results_df::DataFrame,
    ...
    # θ_idx = Dict(g => findfirst(==(Symbol("shock_x_ind", g)), coef_names)
    #              for g in 2:13)
    β_indices = Dict(g => findfirst(==(Symbol("shock_x_ind", g)), coef_names)
                 for g in 1:13)
...
#             θ_sub = filter(r -> r.horizon == h && r.coef_name == Symbol("shock_x_ind", g), results_df)
...
#     end
    for g in 1:13
        ...
        for h in horizons
            # 方案 A：直接找 shock_x_ind_g 的系数（不再有独立的 :shock）
